...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let visibleCount = 5; // Reduced to 5 for better initial view
    const increment = 5;
    ...
    function applyFilters(){
        ...
        serviceCards.forEach(card => {
            ...
            const isMatch = (selectedSocial === 'all' || s === selectedSocial) && (selectedCategory === 'all' || c === selectedCategory) && matchesSearch;
            
            if (isMatch) {
                matches.push(card);
            }
            card.style.display = 'none'; // Default to hide
        });

        // Then show only the visible portion of matches
        matches.slice(0, visibleCount).forEach(card => {
            card.style.display = 'flex';
        });

        if (loadMoreBtn) {
            loadMoreBtn.style.display = matches.length > visibleCount ? 'block' : 'none';
        }
    }
