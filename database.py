...
class Database:
    ...
    def get_all_games_with_id(self):
        """Returns all games/apps with rowid as item id."""
        try:
            conn = sqlite3.connect(config.DB_NAME_APPS)
            cursor = conn.cursor()
            ...
            cursor.execute(
                "SELECT rowid, name, url, content_type, category, version, updated, genre, developer, size "
    ...
    def get_game_by_id(self, item_id):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT name, url, content_type, category, version, updated, genre, developer, size "
                "FROM apps WHERE rowid = ?",
                (int(item_id),)
            )
            game = cursor.fetchone()
            conn.close()
